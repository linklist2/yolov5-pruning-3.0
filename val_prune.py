...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='data/dior-9.yaml', help='dataset.yaml path')
    # parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5s.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default= 'runs/train/exp33/weights/prune.pt', help='model.pt path(s)')
...
def main(opt):
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        # 如果开启了验证prune后的model的选项，就先根据mask进行剪枝，然后再接着val
        # if opt.prune_val:
        ...
        #     prune(model=model, pruning_strategy=opt.pruning_strategy, is_training=False)
        #     opt.model = model
        #     run(data_dict,
